...
export default function Customers() {
  ...
  const handleSaveCustomer = async () => {
    ...
    const nameTaken = state.customers.some(
      (customer) =>
        customer.name.trim().toLowerCase() === formData.name.trim().toLowerCase() &&
        customer.id !== selectedCustomer?.id
    );
    const phoneTaken = state.customers.some(
      (customer) =>
        customer.phone.trim() === formData.phone.trim() &&
        customer.id !== selectedCustomer?.id
    );
